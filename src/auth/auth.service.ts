...
export class AuthService {
  ...
  public async validateMember(input: LoginDto, setting: SettingDto) {
    ...
    if (!setting.system_status)
      throw new UnauthorizedException('เว็บไซต์ปิดปรับปรุง');
    ...
    if (!member) {
      ...
      if (!member)
        throw new NotFoundException(
          'ไม่พบข้อมูลในระบบ กรุณาตรวจสอบ username password',
        );
    }
    console.log(member);

    if (setting.agent_username.toLowerCase() != member.agent)
      throw new NotFoundException(
        'ไม่พบข้อมูลในระบบ กรุณาตรวจสอบ username password',
      );
    if (setting.company.toLowerCase() != member.company)
      throw new NotFoundException(
        'ไม่พบข้อมูลในระบบ กรุณาตรวจสอบ username password',
      );
    ...
    if (member.password != input.password)
      throw new BadRequestException('ท่านกรอกข้อมูลไม่ถูกต้อง');
  ...
  public async lineNotiPushRegister(
    username: string,
    name: string,
    lastname: string,
    create_from: string,
  ...
  public getTokenForUserAccounting(user: RegisterUserDto): string {
    return this.jwtservice.sign({
