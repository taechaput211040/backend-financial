...
export class NotifyService {
    ...
    public async getSettingByHash(hash: string): Promise<Notify> {
        this.logger.log(hash)
        return await this.notifyRefRepository.findOne({ where: { hash: hash } });

    }
    public async createSetting(input: CreateNotifyDto): Promise<Notify> {
        return await this.notifyRefRepository.save(input);
